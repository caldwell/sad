...
pub fn to_writer<'a, 'b, W:Write, T: Serialize>(writer: W, value: &'a T) -> Result<()> {
    let mut serializer = Serializer::new(writer,
                                         Language{array_lit:  ArrLit("[", "," ,"]"),
                                                  map_lit:    MapLit("{", ": ", ",", "}" ),
                                                  tuple_lit:  TupLit("(", ",",  ")" ), // FIXME: (1) is not a tuple, but (1,) is.
...
impl PythonStringSerializer {
    fn new() -> StringSerializer<'static> {
        ...
        let mut sq_esc = dq_esc.clone();
        sq_esc['"'  as usize] = Esc::No;
        sq_esc['\'' as usize] = Esc::Rep('\'');

        let mut tsq_esc = tdq_esc.clone();
